...
export const usePersonaManagement = ({
    ...
    const handleLoadPersonasData = useCallback(async () => {
        console.log('[usePersonaManagement] handleLoadPersonasData called.');
        if (!mongoBrandId) {
            console.log("[usePersonaManagement] No MongoDB brand ID, skipping personas data load");
            return;
        }
        try {
            console.log(`[usePersonaManagement] Loading personas for brandId: ${mongoBrandId}`);
            const personas = await loadPersonas(mongoBrandId);
            console.log('[usePersonaManagement] Personas loaded from service:', personas);
            dispatchAssets({ type: 'SET_PERSONAS', payload: personas });
            console.log('[usePersonaManagement] Dispatched SET_PERSONAS action.');
        } catch (error) {
            console.error('[usePersonaManagement] Error loading personas:', error);
    ...
    const handleSavePersona = useCallback(async (persona: Persona) => {
        ...
        try {
            ...
            if (mongoBrandId) {
                const newId = await savePersonaToDatabase(personaToSave, mongoBrandId, settings);
    ...
    const handleUpdatePersona = useCallback(async (persona: Persona) => {
        ...
        if (mongoBrandId) {
            updateAutoSaveStatus('saving');
            try {
                await savePersonaToDatabase(persona, mongoBrandId, settings);
                updateAutoSaveStatus('saved');
            } catch (e) {
    ...
    const handleSaveSelectedPersonas = useCallback((selectedPersonas: Partial<Persona>[]) => {
        ...
        const personasToSave: Persona[] = selectedPersonas.map(p => {
            const newId = crypto.randomUUID();
            return {
                id: newId,
                nickName: p.nickName || 'New Persona',
                fullName: p.fullName || '',
                background: p.background || '',
                outfitDescription: p.outfitDescription || '',
    ...
    }, [handleSavePersona, mongoBrandId, setSuccessMessage]);
